...
for METHOD in "${METHODS[@]}"; do
  ...
  if [[ "$METHOD" == "dola" || "$METHOD" == "deco" ]]; then
    RESP_FILE="${OUTPUT_DIR}/${MODEL_ID}/${EARLY_EXIT_LAYERS}_layers/${METHOD}/responses_fine-tune.json"
    METRIC_FILE="${OUTPUT_DIR}/${MODEL_ID}/${EARLY_EXIT_LAYERS}_layers/${METHOD}/metric_fine-tune.json"
  else
